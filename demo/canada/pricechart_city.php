...
$SQL = "SELECT
		aie_masterdata.Price as price,
		aie_masterdata.DepartureDate as weekdate
		FROM
		aie_masterdata
		WHERE
		aie_masterdata.DepartureDate <= '".date ( "Y-m-d 23:59:59", $Query_DepartureDate)."' AND
		...
		aie_masterdata.DepartureCity = '".$DepartureCityCode."' AND 
		aie_masterdata.ArrivalCity = '".$ArrivalCityCode."'		
		ORDER BY aie_masterdata.DepartureDate ASC ";
...
if ($Type == "YEAR") // 一个月内
{
		$graph->title->Set ($DepartureCity."至". $ArrivalCity."价格年走势图" );
}
else // MONTH 一年内
{
		$graph->title->Set ($DepartureCity."至". $ArrivalCity."价格月走势图" );
}
...
$graph->yaxis->title->SetFont ( FF_SIMSUN, FS_NORMAL, 10 );
