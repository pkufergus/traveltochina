	...
	ETravel.prototype = 
	{
		...
		keepCalendarGap: function(fromCal, days)
		{
			...
			if((fromCal == "#divB_DATE1"  || fromCal ==  "#divE_DATE2") && SelectTab=="R" )
			{
				...
				var fromdate = etravel.dateStr1(etravel.getDateTimeComposed('#divB_DATE1'));
				var todate = etravel.dateStr1(etravel.getDateTimeComposed('#divE_DATE2'));
				
				if (to != "Beiging" && to != "BJS"  && to != "Shanghai" && to != "SHA" )
				{
					return;
				}
				if (to != "Beiging" && to != "BJS"  && to != "Shanghai" && to != "SHA" )
				...
				if(from == "Los Angeles")
					...
					tempFrom = "NYC";
				else if (from == "San Francisco") 
					tempFrom = "SFO";
				else if (from == "Boston") 
					tempFrom = "BOS";
				else if (from == "Chicago") 
					tempFrom = "CHI";
				else if (from == "Washington") 
					tempFrom = "WAS";
				else if (from == "Seattle") 
					tempFrom = "SEA";
				else
					tempFrom = from;	
					
				parent.document.getElementById("FROM").value=tempFrom;
				parent.document.getElementById("TO").value=to;
				window.parent.selectchang();

				parent.document.getElementById("pricechart_month").src = "pricechart_city.php?type=MONTH&from="+from+"&to="+tempTo+"&fromdate="+fromdate+"&todate="+todate;
				parent.document.getElementById("pricechart_year").src = "pricechart_city.php?type=YEAR&from="+from+"&to="+tempTo+"&fromdate="+fromdate+"&todate="+todate;
				
				var stay = document.getElementById("stay_length").value; 
				var calendar_checked = document.getElementById("calendar_sel_exact2").checked;
				if(stay != "" &&calendar_checked)
				{
					parent.document.getElementById("calendar-view").src = "calendar_of_lowest_fares.php?stay="+stay+"&from="+from+"&to="+to+"&fromdate="+fromdate+"";
				}
			}
...
function selectChange(formid,formid2)
{
	...
	{
		if((formid2 == "B_LOCATION_1"  || formid2 ==  "E_LOCATION_1") && SelectTab=="R" )
		{
		    var from = document.getElementById("B_LOCATION_1").value;
			var to = document.getElementById("E_LOCATION_1").value;
			...
			var fromdate = etravel.dateStr1(etravel.getDateTimeComposed('#divB_DATE1'));
			var todate = etravel.dateStr1(etravel.getDateTimeComposed('#divE_DATE2'));
			//&from=New York&to=Beijing&fromdate=2011-11-25&todate=2011-12-02
			//&from=New York&to=Dalian&fromdate=2011-11-26&todate=2011-12-03
			parent.document.getElementById("pricechart_month").src = "pricechart_city.php?type=MONTH&from="+from+"&to="+tempTo+"&fromdate="+fromdate+"&todate="+todate;
			parent.document.getElementById("pricechart_year").src = "pricechart_city.php?type=YEAR&from="+from+"&to="+tempTo+"&fromdate="+fromdate+"&todate="+todate;
			var stay = document.getElementById("stay_length").value; 
			var calendar_checked = document.getElementById("calendar_sel_exact2").checked;
